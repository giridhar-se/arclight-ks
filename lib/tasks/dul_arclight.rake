...
# Based on the core index rake task but adjusted for DUL traject config, see:
  ...
  task index_dir: :environment do
    ...
    files = Dir.glob(File.join(ENV.fetch('DIR', nil), '*.xml'))
    solr_url = ENV.fetch('SOLR_URL', nil)

    Parallel.each(files, in_threads: Etc.nprocessors) do |file|
      puts "Loading #{file} into index..."
      elapsed_time = Benchmark.realtime do
        `bundle exec traject -u #{solr_url} -i xml -c ./lib/traject/dul_ead2_config.rb #{file}`
      end
      puts "Indexed #{file} (in #{elapsed_time.round(3)} secs)."
    end
  end
